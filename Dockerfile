...
RUN pip install --user --ignore-installed \
    cryptography defusedxml requests "apsw >= 3.7.0" "trio >= 0.15" "pyfuse3 >= 3.0, < 4.0" "dugong >= 3.4, < 4.0" google-auth google-auth-oauthlib
RUN gpg2 --batch --keyserver hkps://keys.openpgp.org --recv-key 0xD113FCAC3C4E599F
ARG S3QL_VERSION
